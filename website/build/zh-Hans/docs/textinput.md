...
Note that `submitBehavior` now takes the place of `blurOnSubmit` and will override any behavior defined by `blurOnSubmit`. See [submitBehavior](/zh-Hans/docs/textinput.md#submitbehavior).
...
An optional identifier which links a custom [InputAccessoryView](/zh-Hans/docs/inputaccessoryview.md) to this text input. The InputAccessoryView is rendered above the keyboard when this text input is focused.
...
An optional label that overrides the default [InputAccessoryView](/zh-Hans/docs/inputaccessoryview.md) button label.
...
`numberOfLines` on iOS is only available on the [New Architecture](/zh-Hans/architecture/landing-page.md)
...
If you are attempting to access the `text` value from `nativeEvent` keep in mind that the resulting value you get can be `undefined` which can cause unintended errors. If you are trying to find the last value of TextInput, you can use the [`onEndEditing`](/zh-Hans/docs/textinput.md#onendediting) event, which is fired upon completion of editing.
...
[`autoComplete`](#autocomplete), provides the same functionality and is available for all platforms. You can use [`Platform.select`](/zh-Hans/docs/next/platform#select) for differing platform behaviors.
...
[Styles](/zh-Hans/docs/style.md)

| Type                                |
| ----------------------------------- |
| [Text](/zh-Hans/docs/text.md#style) |
