...
In [Integrating with Existing Apps guide](/zh-Hans/docs/integration-with-existing-apps.md) and [Native UI Components guide](/zh-Hans/docs/legacy/native-components-android.md) we learn how to embed React Native in a native component and vice versa. When we mix native and React Native components, we'll eventually find a need to communicate between these two worlds. Some ways to achieve that have been already mentioned in other guides. This article summarizes available techniques.
...
The problem exposing properties of native components is covered in detail in [this article](/zh-Hans/docs/legacy/native-components-android.md#3-expose-view-property-setters-using-reactprop-or-reactpropgroup-annotation). In short, properties that are to be reflected in JavaScript needs to be exposed as setter method annotated with `@ReactProp`, then use them in React Native as if the component was an ordinary React Native component.
...
Although we have a flavor of cross-language callbacks ([described here](/zh-Hans/docs/legacy/native-modules-android.md#callbacks)), these callbacks are not always the thing we need. The main problem is that they are not intended to be passed as properties. Rather, this mechanism allows us to trigger a native action from JS, and handle the result of that action in JS.
...
Events are described in detail in [this article](/zh-Hans/docs/legacy/native-components-android.md#events). Note that using events gives us no guarantees about execution time, as the event is handled on a separate thread.
...
Native modules are Java/Kotlin classes that are available in JS. Typically one instance of each module is created per JS bridge. They can export arbitrary functions and constants to React Native. They have been covered in detail in [this article](/zh-Hans/docs/legacy/native-modules-android.md).
